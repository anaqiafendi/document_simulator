...
export async function listFakerProviders(): Promise<FakerProvidersResponse> {
  const r = await fetch(`${BASE}/api/synthesis/faker-providers`)
  if (!r.ok) throw new Error(`Failed to fetch faker providers: ${r.status}`)
  return r.json()
}
